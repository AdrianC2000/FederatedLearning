...
class FederatedConfig:
    def __init__(self, he_config: HEConfig, num_clients: int = NUM_CLIENTS, num_rounds: int = NUM_ROUNDS,
...
class FederatedTrainer(BaseFederatedTrainer):
    def __init__(self, train_loader: DataLoader, test_loader: DataLoader, model_fn: callable, config: FederatedConfig):
        ...
        self.he_context = ts.context(
            ts.SCHEME_TYPE.CKKS,
            poly_modulus_degree=config.he_config.poly_modulus_degree,
            coeff_mod_bit_sizes=config.he_config.coeff_mod_bit_sizes
        )
        self.he_context.global_scale = config.he_config.scale
    ...
    def train(self) -> ModelWrapper:
        ...
        print(f"\n##### Running FL with HE | clients={self.config.num_clients}, rounds={self.config.num_rounds}, "
              f"epochs={self.config.local_epochs}, lr={self.config.learning_rate}, "
    ...
    def _train_single_client(self, index: int) -> Dict[str, ts.CKKSVector]:
        ...
        client_config = ClientConfig(
            context=self.he_context, epochs=self.config.local_epochs, learning_rate=self.config.learning_rate,
    ...
    def _aggregate_fed_opt(self, encrypted_weights_list: List[Dict[str, ts.CKKSVector]], method: AggregationMethod) -> None:
    ...
    def _decrypt_and_average(self, encrypted_list: List[Dict[str, ts.CKKSVector]]) -> Dict[str, torch.Tensor]:
        ...
        for key in encrypted_list[0]:
            ...
            agg *= self.client_num_samples[0] / total_samples

            for update, n in zip(encrypted_list[1:], self.client_num_samples[1:]):
                agg += update[key] * (n / total_samples)
