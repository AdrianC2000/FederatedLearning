...
from trainers.federated_smpc.client_trainer import ClientTrainer, ClientConfig, SMPCConfig


class FederatedConfig:
    def __init__(self, smpc_config: SMPCConfig, num_clients: int = NUM_CLIENTS, num_rounds: int = NUM_ROUNDS,
...
class FederatedTrainer(BaseFederatedTrainer):
    def __init__(self, train_loader: DataLoader, test_loader: DataLoader, model_fn: callable, config: FederatedConfig):
        ...
        self.clients = [
            ClientTrainer(
                ...
                ClientConfig(
                    smpc_config=config.smpc_config,
        ...
        self.dropped_client_index = random.randint(0, config.num_clients - 1) if config.smpc_config.drop_clients else None
    ...
    def train(self) -> ModelWrapper:
        print(
            f"\n##### Running Federated Learning with SMPC | "
            ...
            f"share_noise_scale={self.config.smpc_config.share_noise_scale}, "
            f"drop_clients={self.config.smpc_config.drop_clients} #####\n"
        )
        ...
        for round_num in range(self.config.num_rounds):
            ...
            self._generate_shares(active_clients)
            self._simulate_share_exchange(active_clients)
            self._compute_masked_updates(active_clients)
            self._aggregate_masked_models(active_clients)
            self._evaluate_global_model(active_clients)

        exec_time = time.time() - start_time
        return ModelWrapper(self.global_model, self.train_acc_history, self.train_loss_history,
    ...
    @staticmethod
    def _generate_shares(clients: List[ClientTrainer]):
        for client in clients:
            client.generate_shares()

    @staticmethod
    def _simulate_share_exchange(clients: List[ClientTrainer]):
        for sender in clients:
            for receiver in clients:
                if sender.client_index != receiver.client_index:
                    receiver.receive_share(sender.client_index, sender.shares[receiver.client_index])

    @staticmethod
    def _compute_masked_updates(clients: List[ClientTrainer]):
        for client in clients:
            client.compute_masked_update()
