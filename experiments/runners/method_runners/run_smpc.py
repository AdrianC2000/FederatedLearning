...
from trainers.federated_smpc.client_trainer import SMPCConfig
from trainers.federated_smpc.federated_trainer import FederatedTrainer, FederatedConfig
...
def run_smpc_experiments(
    ...
    privacy_method = "Secure Multi-Party Computation"
    ...
    for drop_clients in drop_clients_values:
        ...
        for noise in noise_scales:
            config_label = f"share_noise_scale_{str(noise).replace('.', '_')}"
            print(f"→ Running SMPC config: {config_label} | {client_group.upper()}")

            test_acc_runs, test_loss_runs, exec_times = [], [], []

            for run_idx in range(num_runs):
                run_seed = seed + run_idx
                print(f"[SMPC] Run {run_idx + 1}/{num_runs} | seed = {run_seed}")
                ...
                trainer = FederatedTrainer(
                    ...
                    model_fn,
                    FederatedConfig(
                        smpc_config=SMPCConfig(share_noise_scale=noise, drop_clients=drop_clients),
            ...
            FLResultUtils.save_metadata(
                config={"share_noise_scale": noise, "drop_clients": drop_clients,
        ...
        print(f"[SMPC] Saved summary to {summary_path}")
